...
```json
{
  ...
  "ignored": [],          // Repositories to exclude from validation
...
Create script to validate metadata consistency:
...
declare -a files=("meta.json" "settings.json" "config/labels.json" "config/milestones.json" "config/repo.json")
for file in "${files[@]}"; do
  if [ ! -f ".chimera-lab/$file" ]; then
    echo "❌ Missing: .chimera-lab/$file"
    ((ERRORS++))
  else
    echo "✅ Found: $file"
  fi
...
for json_file in $(find .chimera-lab -name "*.json"); do
...
if ! jq '.organization.name' .chimera-lab/meta.json > /dev/null; then
  echo "❌ Missing organization.name in meta.json"
  ((ERRORS++))
fi
...
1. **Edit** `.chimera-lab/config/labels.json`:
   ```json
   {
...
3. **Commit**:
   ```bash
   git add .chimera-lab/config/labels.json
   git commit -m "chore(labels): add type:new-feature label"
   ```

4. **Update documentation**:
   - Add to [NAMING_CONVENTIONS.md](../docs/NAMING_CONVENTIONS.md)
   - Update labels reference

### Creating New Milestones

1. **Plan milestones** for coming quarter:
   ```json
   {
     "name": "Q2 2024 Release",
     "description": "Second quarter 2024 release",
     "due_date": "2024-06-30"
   }
   ```

2. **Add to** `.chimera-lab/config/milestones.json`

3. **Announce** in organization discussion

4. **Apply** to affected repositories:
   ```bash
   # Use GitHub CLI
   gh milestone create "Q2 2024 Release" \
     --due-date 2024-06-30 \
     -R chimera-lab/repository-name
   ```

### Updating Repository Configuration

Edit `.chimera-lab/config/repo.json`:
...
### Apply Labels to Repositories

Using GitHub CLI:

```bash
#!/bin/bash
# sync-labels.sh - Apply organization labels to all repositories

ORG="chimera-lab"
LABELS_FILE=".chimera-lab/config/labels.json"

# Get all label names from file
LABEL_NAMES=$(jq -r '.[].labels[]? | .name' "$LABELS_FILE")

for repo in $(gh repo list $ORG --json nameWithOwner -q '.[].nameWithOwner'); do
  echo "Syncing labels to $repo..."
  
  jq -r '.[] | .labels[]? | "\(.name)|\(.description)|\(.color)"' "$LABELS_FILE" | while IFS='|' read -r name desc color; do
    gh label create "$name" \
      --description "$desc" \
      --color "$color" \
      -R "$repo" 2>/dev/null || echo "  - $name (exists)"
  done
done
```

### Apply Milestones to Repositories
...
ORG="chimera-lab"
MILESTONES_FILE=".chimera-lab/config/milestones.json"

for repo in $(gh repo list $ORG --json nameWithOwner -q '.[].nameWithOwner'); do
  echo "Creating milestones in $repo..."
  
  jq -r '.templates[] | "\(.name)|\(.description)|\(.due_date)"' "$MILESTONES_FILE" | while IFS='|' read -r name desc due_date; do
    gh milestone create "$name" \
      --description "$desc" \
      --due-date "$due_date" \
      -R "$repo" 2>/dev/null || echo "  - $name (exists)"
  done
...
Track metadata changes in `.chimera-lab/CHANGELOG.md`:
